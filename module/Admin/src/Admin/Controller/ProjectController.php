...
class ProjectController extends MainController {
							...
							if ($projectService->add ( $prj )) {
								...
								if ($this->createDatabase ( $prj->projectName )) {
									...
									if($config){
										...
										$geoserver->host = $config["geoserver"]["host"];
									...
									if($datasourceService->add($datasource)){
										if($geoserverService->addGeoserver($geoserver)){
											...
											$this->deleteDatabase($prj->projectName);
											$datasourceService->rollback($prj->projectName);
										}
									}
									else{
										$this->deleteDatabase($prj->projectName);
								...
								if ($result) {
									...
									if($saveLogo){
										$pathLogo = getCwd() . '/module/Workspace/src/Workspace/file-uploads/logos/'.$prj->prjId.".jpg";
										unlink ($pathLogo);
									}
									if($sldPath)
										unlink ( $sldPath );
	...
	public function createDatabase($prjName){
		...
		$datasourceService = $serviceLocator->get ( 'Storage\Service\DataSourceService' );
		$config = $this->getConfiguration();
		if(pg_connect('host='.$config["datasource"]["host"].' user='.$config["datasource"]["login"].' password='.$config["datasource"]["password"].' connect_timeout=5')){
			$sql = "CREATE DATABASE ". $prjName;
			$query = pg_query($sql);
			if($query && pg_connect('host='.$config["datasource"]["host"].' dbname='.strtolower($prjName).' user='.$config["datasource"]["login"].' password='.$config["datasource"]["password"].' connect_timeout=5')){
				$sqlPostgis = "CREATE EXTENSION postgis";
				$queryPostgis = pg_query($sqlPostgis);
				if ($queryPostgis){
					$sqlTopology = "CREATE EXTENSION postgis_topology";
					$queryTopology = pg_query($sqlTopology);
					if ($queryTopology){
						pg_close();
						return true;					
					}else{
			...
			if ($this->verifyUserSession ()) {
				...
				if ($acl->isAllowed ( $this->session->user->rol->name, "Administração", "Administrar usuários e permissões" )) {
					...
					if ($projectService->setActive ( $prj, 0 )) {
						$this->showMessage ( 'O projeto foi desativado com sucesso!', 'admin-success' );
						$response->setContent ( \Zend\Json\Json::encode ( array (
								'status' => true,
								'isLogged' => true 
						) ) );
			...
			if ($this->verifyUserSession ()) {
				...
				if ($acl->isAllowed ( $this->session->user->rol->name, "Administração", "Administrar usuários e permissões" )) {
					...
					if ($prj && $projectService->setActive ( $prj, 1 )) {
						$this->showMessage ( 'O projeto foi ativado com sucesso!', 'admin-success' );
						$response->setContent ( \Zend\Json\Json::encode ( array (
								'status' => true,
								'isLogged' => true 
						) ) );
			...
			if ($this->verifyUserSession ()) {
				...
				if ($acl->isAllowed ( $this->session->user->rol->name, "Administração", "Administrar usuários e permissões" )) {
					...
					if($shapes){
						...
						if ($prj && $projectService->publish($prj,$date,$datasource,$tableName)) {
							$response->setContent ( \Zend\Json\Json::encode ( array (
									'status' => true,
									'isLogged' => true,
